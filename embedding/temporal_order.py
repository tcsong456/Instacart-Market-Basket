...
def reorder_data_maker(data,max_len,mode='train'):
    ...
    with tqdm(total=data.shape[0],desc='building reorder data for dataloader',dynamic_ncols=True,
        ...
        for _,row in data.iterrows():
            ...
            new_seen_ord = []
            reorder_ratio_ord = []
            in_reorder_trend_ord = []
            reorder_size_ratio_ord = []
            for idx,(reorder,aisle) in enumerate(zip(reorders,aisles)):
                in_reorder = max(reorder) == 1
                total_in_reorder += in_reorder
                in_reorder_trend = total_in_reorder / (idx + 1)
            ...
            reorder_info = np.stack([in_reorder_ord,in_reorder_trend_ord,np.array(reorder_sum_ord)/100,np.array(order_size_ord)/145,
                                     reorder_ratio_ord,reorder_size_ratio_ord,reorder_trend_ratio_ord,new_seen_ord,days_interval/30,
                                     next_in_reorder],axis=1).astype(np.float16)
